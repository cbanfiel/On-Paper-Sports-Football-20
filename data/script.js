...
//football sliders
export let qbCompletionSlider = 42;
...
export let trainingPointsAvailable = 3;
...
export function getTradeFinderOffers(offer) {
    ...
    let offerValue = 0;
    let offers = [];
    //fixes cpu teams always offering kickers
    let offerIncludesK = false;
    let offerIncludesP = false;
    for (let i = 0; i < offer.length; i++) {
        if (offer[i].isPick === true) {
            getDraftPickProjectedPick(offer[i]);
        }
        if (offer[i].position == POS_K) {
            offerIncludesK = true;
        }
        if (offer[i].position == POS_P) {
            offerIncludesP = true;
        }
        ...
        if (teams[i] != selectedTeam) {
            ...
            if (Math.random() * 100 > 40) {
                for (let j = 0; j < teams[i].roster.length; j++) {
                    ...
                    if (ply.position == POS_K && !offerIncludesK) {
                        continue;
                    }
                    if (ply.position == POS_P && !offerIncludesP) {
                        continue;
                    }
